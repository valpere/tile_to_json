...
A high-performance command-line tool for converting Mapbox Vector Tiles (MVT) from Protocol Buffer format to JSON/GeoJSON format.
...
# Using coordinates and base URL
...
tile-to-json batch --zoom 14 --bbox "-74.0,40.7,-73.9,40.8" --output-dir ./tiles/

# Combine all tiles into single file
tile-to-json batch --zoom 10 --bbox "-74.0,40.7,-73.9,40.8" --output tiles.geojson --single-file
```
...
|------|-------------|----------|
| `--url` | Direct URL to the tile | Either `--url` or coordinates |
| `--z` | Tile zoom level | Either `--url` or coordinates |
| `--x` | Tile x coordinate | Either `--url` or coordinates |
| `--y` | Tile y coordinate | Either `--url` or coordinates |
| `--output, -o` | Output file path (default: stdout) | No |
| `--compression` | Compress output file | No |
